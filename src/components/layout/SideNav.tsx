import { Settings, Rocket, SlidersHorizontal, FileText, ChevronDown, PanelLeftClose, PanelLeftOpen, MousePointerClick } from 'lucide-react';
...
export function SideNav({
  ...
  return (
    <>
      ...
      >
        <button
          className={`nav-item ${page === 'session-json' && !shouldLockActiveOnMore ? 'active' : ''}`}
          onClick={() => setPage('session-json')}
          title="点这里：转JSON"
        >
          <MousePointerClick size={isClassicLayout ? classicMainIconSize : 20} />
          {showClassicLabels ? (
            <span className="nav-item-text">转JSON</span>
          ) : !isClassicLayout ? (
            <span className="tooltip">点这里：转JSON</span>
          ) : null}
        </button>
